...
```javascript
/*
Sabemos que tenemos n bits y por tanto cuantos numeros se pueden representar con ellos, una mitad permite representar los numeros positivos y el 0, pero el 0 tambien se debe representar entonces al max value le debemos restar 1, y para hallar el minvalue no hay necesidad de restar.
*/
function limits(n) {
 return {
  maxval: ((2**n)/2)-1,
